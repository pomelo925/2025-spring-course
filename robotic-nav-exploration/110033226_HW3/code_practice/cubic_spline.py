...
def ensure_unique_coordinates(path):
    x_values = set()
    y_values = set()
    for i in range(len(path)):
        while path[i][0] in x_values:  # 確保 x 唯一
            path[i][0] += 1
        while path[i][1] in y_values:  # 確保 y 唯一
            path[i][1] += 1
        x_values.add(path[i][0])
        y_values.add(path[i][1])
    return path
...
def cubic_spline_2d(path, interval=2):
    path = ensure_unique_coordinates(path)
