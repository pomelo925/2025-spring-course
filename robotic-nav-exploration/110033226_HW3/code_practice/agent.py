...
class PPO:
    ...
    def train(self, mb_states, mb_actions, mb_old_values, mb_advs, mb_returns, mb_old_a_logps):
        # 將 numpy 陣列轉成 PyTorch tensor，並移動到指定裝置上
        ...
        for i in range(self.sample_n_epoch):
            ...
            for j in range(self.sample_n_mb):
                ...
                # 從 policy_net 得到 log prob 和 entropy
                ...
                # Critic loss (value function loss) 使用 clipped loss
                ...
                # unclipped 和 clipped policy gradient loss
    ...
    # Learning rate decay (applies to both actor and critic)
